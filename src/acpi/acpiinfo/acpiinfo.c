...
static void acpiinfo_check(fwts_log *log, char *line, char *prevline, void *private, int *warnings, int *errors)
{
	...
	if (strstr(line, "ACPI: Subsystem revision")!=NULL) {
		...
		if (version) {
			version+=5;
			fwts_log_info(log, "Linux ACPI interpreter version %s", version);
		}
	}
	...
	if (strstr(line, "ACPI: DSDT")!=NULL) {
		...
		if (strstr(line,"INTL"))
			vendor="Intel";
		fwts_log_info(log,"DSDT was compiled by the %s AML compiler", vendor);
	}
...
static int acpiinfo_init(fwts_log *results, fwts_framework *fw)
{
...
static int acpiinfo_deinit(fwts_log *results, fwts_framework *fw)
{
	fwts_klog_free(klog);
...
static int acpiinfo_test1(fwts_log *results, fwts_framework *fw)
{	
	char *test = "General ACPI information check";
	int warnings = 0;
	int errors = 0;

	fwts_log_info(results, "This test checks the output of the in-kernel ACPI CA against common\n"
		 "error messages that indicate a bad interaction with the bios, including\n"
		 "those that point at AML syntax errors.\n");

	if (fwts_klog_scan(results, klog, acpiinfo_check, fw, &warnings, &errors)) {
