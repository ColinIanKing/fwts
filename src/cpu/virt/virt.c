...
static int virt_test1(fwts_framework *fw)
{
	...
	if (is_AMD()) {
		cpu = CPU_AMD;
		fwts_framework_passed(fw, "CPU is an AMD");
	} else if (is_Intel()) {
		cpu = CPU_INTEL;
		fwts_framework_passed(fw, "CPU is an Intel");
	} else {
		cpu = CPU_UNKNOWN;
		fwts_framework_failed(fw, "CPU is unknown");
	}
	return 0;
}
...
static int virt_test2(fwts_framework *fw)
{
	...
	case CPU_INTEL:
		...
		break;
	default:
		fwts_log_warning(fw, "Cannot test virtualisation extentiosn - unknown CPU");
		break;
	}
	return 0;
}
