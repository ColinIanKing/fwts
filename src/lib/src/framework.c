...
static void fwts_framework_syntax(char * const *argv)
{
	...
	static fwts_syntax_info syntax_help[] = {
		...
		{ "-d, --dump",			"Dump out dmesg, dmidecode, lspci, ACPI tables to logs." },
		...
		{ "-S, --skip-test=t1[,t2]",	"Skip tests named t1, t2.." },
...
static void fwts_framework_heading_info(fwts_framework *fw, fwts_list *tests_to_run)
{
	...
	fwts_log_nl(fw);
	fwts_log_info(fw, "Some of this work - Copyright (c) 1999 - 2010, Intel Corp. All rights reserved.");
	fwts_log_info(fw, "Some of this work - Copyright (c) 2010, Canonical.");
	fwts_log_nl(fw);
